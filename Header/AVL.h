...
AVLTree<Key, Value, Comparator>::insert(const Key &key, const Value &value) 
{
	...
	*/
	if (toInsert->mNext) { toInsert->mNext->mPrev = toInsert; }
	else { mTail = toInsert; }

	/* Update the next pointer of the previous entry similarly. */
	if (toInsert->mPrev) { toInsert->mPrev->mNext = toInsert; }
	else { mHead = toInsert; }
	...
	AVLTree<Key, Value, Comparator>::findNode(const Key &key) const 
{
	...
	*/
	...
	while (curr != NULL) 
	{
		...
		if (mComp(key, curr->mValue.first)){ curr = curr->mChildren[0]; }
		/* Otherwise if the key is greater than the node, go right. */
		else if (mComp(curr->mValue.first, key)) { curr = curr->mChildren[1]; }
		...
		*/
		else { return std::make_pair(curr, curr->mParent); }
	}
...
void AVLTree<Key, Value, Comparator>::spliceOut(Node *node) 
{
	...
	*/
	if (parent){ parent->mChildren[node == parent->mChildren[1]] = child; }
...
const Value &AVLTree<Key, Value, Comparator>::at(const Key &key) const 
{
	...
	if (result == end()) { throw std::out_of_range("Key not found in AVL tree."); }
...
AVLTree<Key, Value, Comparator>::lower_bound(const Key &key) const 
{
	...
	*/
	...
	if (result.first) { return iterator(this, result.first); }
	...
	*/
	if (mComp(result.second->mValue.first, key)) { result.second = result.second->mNext; }
